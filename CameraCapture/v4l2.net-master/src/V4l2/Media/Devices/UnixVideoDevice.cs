...
namespace Iot.Device.Media
{
    ...
    internal class UnixVideoDevice : VideoDevice
    {
        ...
        private int Clamp(int value, int upperBound, int lowerBound)
        {
            if (value < lowerBound)
                return lowerBound;
        ...
        public override void SendSerializedFrame(Socket socket, string path)
        {
            Initialize();
            SetVideoConnectionSettings();
            byte[] dataBuffer = ProcessCaptureData();
            ...
            for (int count = 0; count < dataBuffer.Length;)
            {
                ...
                socket.Send(dataBuffer, count, buffer, SocketFlags.None);
                count += buffer;
            }
            
            if(path != "NA")
                SaveFrame(path, dataBuffer);
        }

        public override void SaveFrame(string path, byte[] dataBuffer)
        {
            using FileStream fs = new FileStream(path, FileMode.Create);
            fs.Write(dataBuffer, 0, dataBuffer.Length);
            fs.Flush();
        ...
        public unsafe byte[] ProcessCaptureData()
        {
            fixed (V4l2FrameBuffer* buffers = &ApplyFrameBuffers()[0])
            {
                ...
                var status = Interop.ioctl(_deviceFileDescriptor, (int)RawVideoSettings.VIDIOC_STREAMON, new IntPtr(&type));

                byte[] dataBuffer = GetFrameData(buffers);

                // Close data stream
                status = Interop.ioctl(_deviceFileDescriptor, (int)RawVideoSettings.VIDIOC_STREAMOFF, new IntPtr(&type));
            ...
            where T : struct
        {
            ...
            int result = Interop.ioctl(_deviceFileDescriptor, ioctlCode, ptr);
            int errno = Marshal.GetLastWin32Error();

            Debug.WriteLine(request + " - " + ioctlCode + $" {result.ToString()}/{errno.ToString()}") ;
